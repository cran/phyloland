...
internal_node_height <- function(gtreel){
	inode_height = vector("numeric", length(gtreel[,1]))
	for (n in 1:length(gtreel[,1])) {
		if (is.na(gtreel[n,2]) & is.na(gtreel[n,3])) { ## tip
			inode_height[n] = 0
		} else {
			inode_height[n] = gtreel[gtreel[n,2],4] + height(gtreel[n,2], gtreel)
		}
	}
	return(inode_height)
}
...
height <- function(node, gtreel){
	if (is.na(gtreel[node,2]) & is.na(gtreel[node,3])) {
		...
		return(gtreel[gtreel[node,2],4] + height(gtreel[node,2], gtreel))
	}
}
...
mcmc_phyloland<-function(space, gtreel, simul_values, treelikelihood, est_sigma=c(1,1), est_lambda=1, est_tau=1, sample_geneal=0, sample_loc=0, 
  ...
  Ndim = dim(space)[1]
  tnumi = sum(is.na(gtreel[,2])) ## number of tips
...
PLD_interface <- function(fileTREES, fileDATA, num_step=100000, freq=100, burnin=0, ess_lim=100, sigma=NA, lambda=NA, tau=NA, num_step_sigma=1, num_step_lambda=1, num_step_tau=1, id_filena=NA, pattern_trees_likelihood="treeLikelihood", names_locations=NA){
  ...
  for (n in 1:dim(space)[2]){
	...
	locations_sim = cbind(rep(0,dim(gtreel)[1]),rep(0,dim(gtreel)[1]),rep(0,dim(gtreel)[1]))
	...
	if (is.na(filena_loc)){
  	nodes = c(1:length(gtreel[,1]))
  	nodes[1:length(tips)] = tips
  	write.table(t(c("num_step",nodes)),row.names=FALSE,col.names=FALSE,file=filena_loc,sep="\t",eol="\n",append=FALSE)
	}

	# MCMC
	mcmco = mcmc_phyloland(space=space, gtreel=gtreel, simul_values=list(sigma,lambda,tau,locations_sim), treelikelihood=gtreelikelihood, est_sigma=c(1,1), est_lambda=1, est_tau=1, sample_geneal=0, sample_loc=1, plot_phylo=0, plot_MCMC=0, save_MCMC=0, Nstep=num_step, freq=freq, Nstep_sigma=num_step_sigma, Nstep_lambda=num_step_lambda, Nstep_loc=1, Nstep_genealogy=0, Nstep_tau=num_step_tau, pchange=0, ess_lim=ess_lim, model=4, show_loc=0, filena_loc, filena_tracer, filena_tree)
...
read_loc <- function(fileLOC){
	sampled_loc <- read.table(file=fileLOC, header=TRUE, sep="\t", quote='"', comment.char="", check.names=FALSE)
...
plot_trees_tips<-function(tree_phylo, location, space, show_index=0){
  ...
  if (show_index==1){
    tips = paste(tree_phylo$tip, colnames(location)[as.numeric(sub("t","",tree_phylo$tip))], sep="_")
    tree_phylo$node.label = paste(as.numeric(location[(length(tree_phylo$tip)+1):(length(location))]), nodes[ as.numeric(location[(length(tree_phylo$tip)+1):(length(location))]) ], sep="_")
  }else{
    tips = colnames(location)[as.numeric(sub("t","",tree_phylo$tip))]
    tree_phylo$node.label = nodes[ as.numeric(location[(length(tree_phylo$tip)+1):(length(location))]) ]
  }
...
PLD_stat_mig <- function(x, sub_sample = 0, first = FALSE){
	...
	for (m in 1:length(sampled_loc[,1])){ 
		history = treel2mig(converttree(trees[[m]]),sampled_loc[m,],space)
...
reorder_treel <- function(gtreel,locations=0){
  gtreelO = matrix(0,nrow=nrow(gtreel),ncol=ncol(gtreel))
  locationsO = locations*0 # intialise to 0
  ntips = ( length(gtreel[,1])+1 )/2
  nnodes = ntips-1
  if (!is.na(gtreel[ntips+1,1])){stop('reorder_treel() error: root not following tips')}
  gtreelO[1:ntips,2:3] = cbind(rep(NA,ntips),rep(NA,ntips)) # tips first (no children)
  gtreelO[ntips+1,c(1,4)] = NA # then root (no parent)
  gtreelO[1:ntips,4] = gtreel[1:ntips,4] # height for tips stay the same
  ## get the absolute height of each internal node
  inode_height = internal_node_height(gtreel)
  ...
  nh = cbind(node_id,c(node_id[1:ntips],nrow(gtreel):(rootn+1),rootn))
  locationsO[nh[,2]] = locations[nh[,1]]
  for (n in (ntips+1):nrow(gtreel)) {
    if(gtreel[nh[n,1],2]<=ntips){
      filsg = gtreel[nh[n,1],2]
    }else{
      filsg = nh[which(nh[,1]==gtreel[nh[n,1],2]),2]
    }
    if(gtreel[nh[n,1],3]<=ntips){
      filsd = gtreel[nh[n,1],3]
    }else{
      filsd = nh[which(nh[,1]==gtreel[nh[n,1],3]),2]
    }
    gtreelO[nh[n,2],2:3] = c(filsg,filsd)
    gtreelO[filsg,1] = nh[n,2]
    gtreelO[filsd,1] = nh[n,2]
    gtreelO[filsg,4] = gtreel[gtreel[nh[n,1],2],4]
    gtreelO[filsd,4] = gtreel[gtreel[nh[n,1],3],4]
  }
  return(list(gtreelO,locationsO))
}

# reOrder tree format Ape, so that oldest internal node appear first
# in Ape tree format, tips of the tree are always from 1 to Ntip(tree_phylo) and internal nodes are indexed from Ntip(tree_phylo)+1
reorder_treep <- function(tree_phylo){
  ...
  node_depth = node.depth.edgelength(tree_phylo)
  ## need to treat the nodes from the most recent to the oldest 
  node_id = sort(node_depth,decreasing=TRUE,index.return=TRUE)$ix
